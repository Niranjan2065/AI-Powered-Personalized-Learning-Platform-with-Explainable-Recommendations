 ...
 *
 * New in this version:
 *  1. "Applications" tab with full approve/reject modal — no separate page needed
 *  2. Pending applications alert banner on Overview
 *  3. Pending count badge on Applications tab
 *  4. Search + role filter on Users tab
 *  5. "View Application" button on tutor rows in Users tab
 *  6. Performance tab shows actual quiz stats from /admin/stats
 */
...
// Add the keyframes globally (you can put this in your main CSS file)
// @keyframes spin { to { transform: rotate(360deg); } }

const Badge = ({ children, color = "#6C63FF", bg = "#EDE9FE" }) => (
  <span style={{
    padding: ".15rem .55rem", borderRadius: 99, fontSize: ".68rem",
    fontWeight: 700, background: bg, color, border: `1px solid ${color}22`
  }}>
    {children}
  </span>
);
      ...
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "1.5rem" }}>
          ...
          <button onClick={onClose} style={{
            ...
            cursor: "pointer", fontSize: "1.25rem", color: "var(--text-muted)"
          }}>✕</button>
        </div>
        ...
        <div style={{ display: "flex", gap: ".6rem", marginBottom: "1.25rem", flexWrap: "wrap" }}>
          {app.linkedinUrl && (
            <a href={app.linkedinUrl} target="_blank" rel="noreferrer"
              className="btn btn-outline btn-sm" style={{ fontSize: ".76rem" }}>🔗 LinkedIn</a>
          )}
          {app.portfolioUrl && (
            <a href={app.portfolioUrl} target="_blank" rel="noreferrer"
              className="btn btn-outline btn-sm" style={{ fontSize: ".76rem" }}>🌐 Portfolio</a>
          )}
          {app.resumeUrl && (
            <a
              href={`http://localhost:5000${app.resumeUrl.startsWith('/') ? '' : '/'}${app.resumeUrl}`}
              target="_blank"
              rel="noreferrer"
        ...
        <div style={{
          ...
          <div style={{ fontSize: ".83rem", lineHeight: 1.7 }}>{app.teachingStatement || "Not provided"}</div>
        </div>
        ...
        {canDecide ? (
          <>
            <div style={{ borderTop: "1px solid var(--border)", paddingTop: "1.25rem", marginBottom: "1rem" }}>
              ...
              <div className="form-group">
                <label className="form-label" style={{ fontSize: ".8rem" }}>
                  Internal Note <span style={{ color: "var(--text-muted)", fontWeight: 400 }}>(not shown to applicant)</span>
                </label>
          ...
          /* Already decided — show existing feedback */
          app.adminFeedback && (
...
export default function AdminDashboard() {
  ...
  // ── Load and view application from user row ──────────────────────────────
  const viewApplicationFromUser = useCallback(async (userId) => {
    try {
      // Find the application for this user
      const { data } = await api.get(`/applications?userId=${userId}&limit=1`);
      if (data.data && data.data.length > 0) {
        setSelectedApp(data.data[0]);
        setTab("applications");
      } else {
        toast.info("No application found for this tutor");
      }
    } catch (err) {
            ...
            <div key={s.label} className="card"
              ...
              <div>
                <div style={{
                  ...
                  fontWeight: 600, marginBottom: ".15rem"
                }}>{s.label}</div>
                ...
                <div className="grid-2" style={{ marginBottom: "1.5rem" }}>
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={healthData} layout="vertical"
                          ...
                          <Bar dataKey="Percentage" radius={[0, 4, 4, 0]} barSize={16}>
                            {healthData.map((_, i) => (
                              <Cell key={i} fill={["var(--secondary)", "var(--primary)", "var(--accent)"][i % 3]} />
                            ))}
                          </Bar>
                        </BarChart>
                      </ResponsiveContainer>
                  ...
                  <div className="card" style={{ padding: "1.25rem" }}>
                    ...
                    <div style={{
                      ...
                      fontSize: ".72rem", fontWeight: 600, marginTop: ".5rem"
                    }}>
                      {userRoleData.map(e => (
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(220px,1fr))", gap: "1rem" }}>
                    {users.slice(0, 6).map(u => (
                      <div key={u._id} style={{
                        ...
                        padding: ".5rem", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)"
                      }}>
                        <div style={{
                        ...
                        <span className={`badge ${u.role === "admin" ? "badge-warning" : u.role === "tutor" || u.role === "teacher" ? "badge-success" : "badge-primary"}`}
                          style={{ fontSize: ".6rem" }}>{u.role}</span>
                      </div>
                    ))}
                  </div>
                </div>
                    ...
                    {filteredUsers.length === 0 ? (
                      ...
                      <tr key={u._id} style={{ borderTop: "1px solid var(--border)" }}>
                        ...
                        <td style={{ padding: ".85rem 1.25rem" }}>
                          ...
                          {/* Show tutorStatus pill next to role */}
                          {u.tutorStatus && u.tutorStatus !== "approved" && (
                            <div style={{ marginTop: ".25rem" }}>
                              <StatusBadge status={u.tutorStatus} />
                            </div>
                          )}
                        </td>
                        <td style={{
                          padding: ".85rem 1.25rem", fontSize: ".82rem",
                        ...
                        <td style={{ padding: ".85rem 1.25rem" }}>
                          {u.role !== "admin" && (
                            <div style={{ display: "flex", gap: ".4rem", flexWrap: "wrap" }}>
                              {/* Tutor: view application */}
                              {(u.role === "tutor" || u.role === "teacher") && (
                                <button className="btn btn-outline btn-sm"
                                  style={{ fontSize: ".75rem" }}
                                  onClick={() => viewApplicationFromUser(u._id)}>
                                  📋 Application
                                </button>
                              )}
                              {u.role === "student" && (
                ...
                <div style={{ display: "flex", gap: ".5rem", marginBottom: "1.25rem", flexWrap: "wrap" }}>
                  {APP_TABS.map(t => (
                    <button key={t.key} onClick={() => setAppTab(t.key)}
                      style={{
                        padding: ".45rem 1rem", borderRadius: "var(--radius-sm)", border: "none",
                        cursor: "pointer", fontSize: ".82rem", fontWeight: appTab === t.key ? 700 : 400,
                        background: appTab === t.key ? "var(--primary)" : "var(--bg)",
                        color: appTab === t.key ? "#fff" : "var(--text-secondary)",
                        border: `1px solid ${appTab === t.key ? "var(--primary)" : "var(--border)"}`
                      }}>
                      {t.label}
                      {t.count > 0 && (
                        <span style={{
                          marginLeft: ".35rem", background: appTab === t.key ? "rgba(255,255,255,.25)" : "var(--border)",
                          borderRadius: 99, padding: ".05rem .4rem", fontSize: ".68rem"
                        }}>
                          {t.count}
                        </span>
                      )}
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: ".75rem", marginBottom: "1.25rem" }}>
                  {APP_TABS.map(t => (
                    <div key={t.key} style={{
                      ...
                      borderTop: `3px solid ${STATUS_META[t.key].border}`,
                      cursor: "pointer"
                    }}
                      onClick={() => setAppTab(t.key)}>
                      <div style={{ fontSize: "1.6rem", fontWeight: 800, color: STATUS_META[t.key].color }}>
                        {t.count}
                      </div>
                    ...
                    {filteredApps.map(app => (
                      <div key={app._id} className="card"
                        style={{
                          ...
                          borderLeft: `4px solid ${STATUS_META[app.status]?.border || "var(--border)"}`
                        }}
                ...
                <div className="card" style={{ padding: "1.25rem" }}>
                  ...
                  {students.length === 0 ? (
                    ...
                    <div key={s._id} style={{
                      ...
                      gap: ".6rem", marginBottom: ".7rem"
                    }}>
                      <span style={{ fontSize: "1.1rem" }}>{["🥇", "🥈", "🥉", "4️⃣", "5️⃣"][i]}</span>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontWeight: 600, fontSize: ".85rem" }}>{s.name}</div>
                        <div style={{ fontSize: ".7rem", color: "var(--text-muted)", textTransform: "capitalize" }}>
                          {s.learningLevel} Level
                        </div>
                      </div>
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={popularCoursesData} margin={{ top: 5, right: 5, left: -25, bottom: 5 }}>
                          ...
                          <Bar dataKey="Students" radius={[4, 4, 0, 0]}>
                            {popularCoursesData.map((_, i) => (
                              <Cell key={i} fill={i % 2 === 0 ? "var(--primary)" : "#8B5CF6"} />
                            ))}
                          </Bar>
                        </BarChart>
                      </ResponsiveContainer>
